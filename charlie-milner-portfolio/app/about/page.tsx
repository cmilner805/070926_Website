    ...
    <div className="mx-auto w-full max-w-7xl px-5 pb-20 pt-14 sm:px-8 sm:pb-28 sm:pt-24">
      ...
      <div className="mt-16 grid gap-14 lg:mt-24 lg:grid-cols-12 lg:gap-20">
        ...
        <div className="lg:col-span-7 lg:pt-2"><div className="max-w-2xl space-y-7 text-lg leading-relaxed text-foreground/90 sm:text-xl">
          ...
          <p>I&apos;m Avid Media Composer Certified, and I&apos;m also proficient in Adobe Premiere Pro and DaVinci Resolve. My work spans documentary, narrative film and client projects, with a particular interest in finding the story within the material.</p>
        ...
        <div className="mt-16 border-t border-border/70 pt-6"><h2 className="text-xs uppercase tracking-[0.28em] text-muted-foreground">Certification</h2><div className="mt-7 space-y-7">{certifications.map((cert) => <a key={cert.title} href={cert.certificateHref} target="_blank" rel="noopener noreferrer" className="group flex items-start gap-5"><img src={cert.badge} alt="" className="h-14 w-14 shrink-0 object-contain" /><span><span className="block text-base text-foreground/90">{cert.title}</span><span className="mt-1 block text-sm text-muted-foreground">{cert.issuer}</span><span className="mt-1 block text-xs uppercase tracking-[0.12em] text-muted-foreground/70">Issued {cert.issued}{cert.expires ? ` · Expires ${cert.expires}` : ''}{cert.credentialId ? ` · Credential ID ${cert.credentialId}` : ''}</span><span className="mt-2 block text-sm text-muted-foreground underline underline-offset-4 group-hover:text-[var(--page-accent)]">View certificate</span></span></a>)}</div></div>
        </div>
      </div>
    </div>
  )
