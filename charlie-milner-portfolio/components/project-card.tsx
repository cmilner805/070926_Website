    ...
    >
      <div className="relative aspect-[16/10] w-full overflow-hidden rounded-md">
        ...
        <div className="pointer-events-none absolute inset-0 hidden bg-background/55 opacity-0 backdrop-blur-[1px] transition-opacity duration-500 group-hover:opacity-100 group-focus-visible:opacity-100 sm:block">
          <div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center">
            <h3 className="translate-y-2 font-sans text-lg font-medium uppercase tracking-[0.2em] text-[var(--page-accent,var(--foreground))] transition-transform duration-500 group-hover:translate-y-0">
              {project.title}
            </h3>
            <p className="translate-y-2 font-serif text-base italic text-muted-foreground transition-transform duration-500 delay-75 group-hover:translate-y-0">
              {project.year}
            </p>
            {project.clientWork && (
              <span className="mt-1 translate-y-2 text-[0.65rem] uppercase tracking-[0.28em] text-muted-foreground transition-transform duration-500 delay-100 group-hover:translate-y-0">
                Client work
              </span>
            )}
          </div>
      ...
      <div className="mt-4 flex items-baseline justify-between gap-4">
        <div className="min-w-0">
          <h3 className="truncate text-sm font-medium uppercase tracking-[0.14em] text-foreground">
            {project.title}
          </h3>
